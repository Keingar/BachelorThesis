...
void FOceanFFTCalculator::Calculate(AGameStateBase* OurGameState)
{
    if (!OurGameState) { return; }

    // don't do anything if it was already calculated this frame
    float serverTime = OurGameState->GetServerWorldTimeSeconds() / 2;
    if (CalculatedEngineTime >= serverTime) return;

    OCEAN_SCOPE_CYCLE_COUNTER(OceanCalculate);

    float AnimationTime = FMath::Fmod(serverTime, OceanData.RepeatPeriod);

    CalculateGridTimeStep(AnimationTime);
    CalculateRowPasses();
    CalculateColPasses();

    CalculatedEngineTime = serverTime;
}
...
{
    double PatchLengtht = OceanData.PatchLength[CascadeIndex] * 150;

    float U = FMath::Frac(PointLocation.X / PatchLengtht / CentimetersPerMeter);
    float V = FMath::Frac(PointLocation.Y / PatchLengtht / CentimetersPerMeter);
